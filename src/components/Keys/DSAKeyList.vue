<template>
    <div class="navItem" v-loading="createdLoading">
        <el-card>
            <el-row>
                <el-col :span="10">
                    <div style="margin-top:20px;" class="boxStyle">本页用于管理DSA密钥对,包括生成,删除和设置访问控制码</div>
                </el-col>
                <el-col :span="14">
        ...
        <el-card>
            <el-table ref="symmetricTable" v-loading="loading" :data="DSAkeyList" :span-method="objectSpanMethod" tooltip-effect="dark" :header-cell-style="{
            'text-align': 'center',
            'color': '#333',
            'font-size': '14px',
                ...
                <el-row :gutter="20" style="margin-top:10px" v-if="isImport == '' && isImport !='2'">
                    <el-col :offset="2">
                        ...
                        <el-form-item label="密钥模长:" label-width="17%" prop="uiKeyBits">
                            ...
                            <el-radio style="margin-left:10px" v-model="form.uiKeyBits" :label="uiKeyBitsLabel2" v-if="uiKeyBitsLabel2!=''">{{uiKeyBitsLabel2}}</el-radio>
                            <el-radio style="margin-left:10px" v-model="form.uiKeyBits" :label="uiKeyBitsLabel3" v-if="uiKeyBitsLabel3!=''">{{uiKeyBitsLabel3}}</el-radio>
                            <el-radio style="margin-left:10px" v-model="form.uiKeyBits" :label="uiKeyBitsLabel4" v-if="uiKeyBitsLabel4!=''">{{uiKeyBitsLabel4}}</el-radio>
                            <el-radio style="margin-left:10px" v-model="form.uiKeyBits" :label="uiKeyBitsLabel5" v-if="uiKeyBitsLabel5!=''">{{uiKeyBitsLabel5}}</el-radio>
                            <el-radio style="margin-left:10px" v-model="form.uiKeyBits" :label="uiKeyBitsLabel6" v-if="uiKeyBitsLabel6!=''">{{uiKeyBitsLabel6}}</el-radio>
...
export default {
    ...
    data() {
        return {
            ...
            uiKeyBitsLabel2: "256",
            uiKeyBitsLabel3: "512",
            uiKeyBitsLabel4: "1024",
            uiKeyBitsLabel5: "2048",
            uiKeyBitsLabel6: "3072"
        }
    },
    created() {
        this.getKeyState(1)
        ...
        this.$commonJs.deviceConfGet().then((res) => {
            this.total = res.data.data.dsa_num
        })
    ...
    methods: {
            ...
            this.$commonJs
                ...
                .then((res) => {
                    if (res.data.code == 100000) {
                        ...
                        // this.DSAkeyList = res.data.data;
                        dataList.forEach((element) => {
                            this.DSAkeyList.push({
        ...
        deleteKey(row) {
            this.$confirm("是否永久销毁DSA密钥?", "提示", {
                ...
                .then((res) => {
                    this.$commonJs
                        .getMethodData(this.$url.DelDSAKey, "POST", {
                            // uiKeyIndex - 密钥内部索引号
