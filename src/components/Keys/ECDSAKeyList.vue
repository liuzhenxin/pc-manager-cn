<template>
    <div class="navItem" v-loading="createdLoading">
        <el-card>
            <el-row>
                <el-col :span="10">
                    <div style="margin-top:20px;" class="boxStyle">本页用于管理ECDSA密钥对,包括生成,删除和设置访问控制码</div>
                </el-col>
                <el-col :span="14">
        ...
        <el-card>
            <el-table ref="symmetricTable" v-loading="loading" :data="ECDSAkeyList" :span-method="objectSpanMethod" tooltip-effect="dark" :header-cell-style="{
            'text-align': 'center',
            'color': '#333',
            'font-size': '14px',
                ...
                <el-row :gutter="20" style="margin-top:10px" v-if="isImport == '' && isImport !='2'">
                    <el-col :offset="2">
                        ...
                        <el-form-item label="密钥模长:" label-width="17%" prop="uiKeyBits">
                            ...
                            <el-radio style="margin-left:10px" v-model="form.uiKeyBits" :label="uiKeyBitsLabel1" v-if="uiKeyBitsLabel1!=''">{{uiKeyBitsLabel1}}</el-radio>
                            <el-radio v-model="form.uiKeyBits" :label="uiKeyBitsLabel2" v-if="uiKeyBitsLabel2!=''">{{uiKeyBitsLabel2}}</el-radio>
                            <el-radio v-model="form.uiKeyBits" :label="uiKeyBitsLabel3" v-if="uiKeyBitsLabel3!=''">{{uiKeyBitsLabel3}}</el-radio>
                            <el-radio v-model="form.uiKeyBits" :label="uiKeyBitsLabel4" v-if="uiKeyBitsLabel4!=''">{{uiKeyBitsLabel4}}</el-radio>
                            <el-radio v-model="form.uiKeyBits" :label="uiKeyBitsLabel5" v-if="uiKeyBitsLabel5!=''">{{uiKeyBitsLabel5}}</el-radio>
                            <el-radio v-model="form.uiKeyBits" :label="uiKeyBitsLabel6" v-if="uiKeyBitsLabel6!=''">{{uiKeyBitsLabel6}}</el-radio>
                            <el-radio v-model="form.uiKeyBits" :label="uiKeyBitsLabel7" v-if="uiKeyBitsLabel7!=''">{{uiKeyBitsLabel7}}</el-radio>
                            <el-radio v-model="form.uiKeyBits" :label="uiKeyBitsLabel8" v-if="uiKeyBitsLabel8!=''">{{uiKeyBitsLabel8}}</el-radio>
                ...
                <el-row :gutter="20" style="margin-top:10px" v-if="Isbatch!=1">
                    <el-col :offset="2">
                        <el-form-item label="密钥用途:" label-width="17%" prop="type">
                            <el-radio style="margin-left:10px" v-model="form.type" label="ecdsa">签名</el-radio>
                            <!-- <el-radio v-model="form.type" label="enc_sm2">加密</el-radio>
                            <el-radio v-model="form.type" label="sm2" v-if="isImport !='1' && isImport !='4'">签名和加密</el-radio> -->
                        </el-form-item>
                    </el-col>
                </el-row>
...
export default {
    ...
    created() {
        ...
        this.$commonJs.deviceConfGet().then((res) => {
            this.total = res.data.data.ecdsa_num
        })
    ...
    methods: {
            ...
            this.$commonJs
                ...
                .then((res) => {
                    if (res.data.code == 100000) {
                        ...
                        // this.ECDSAkeyList = res.data.data;
                        dataList.forEach((element) => {
                            this.ECDSAkeyList.push({
        ...
        deleteKey(row) {
            this.$confirm("是否永久销毁ECDSA密钥?", "提示", {
                ...
                .then((res) => {
                    this.$commonJs
                        .getMethodData(this.$url.DelECDSAKey, "POST", {
                            // uiKeyIndex - 密钥内部索引号
