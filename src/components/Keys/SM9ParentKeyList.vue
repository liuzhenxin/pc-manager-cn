<template>
    <div class="navItem" v-loading="createdLoading">
        <el-card>
            <el-row>
                <el-col :span="10">
                    <div style="margin-top:20px;" class="boxStyle">本页用于管理SM9主密钥对,包括生成,删除和设置访问控制码</div>
                </el-col>
                <el-col :span="14">
        ...
        <el-card>
            <el-table ref="symmetricTable" v-loading="loading" :data="SM9MasterkeyList" :span-method="objectSpanMethod" tooltip-effect="dark" :header-cell-style="{
            'text-align': 'center',
            'color': '#333',
            'font-size': '14px',
                ...
                <el-table-column label="用途">
                    <template slot-scope="scope">
                        <span v-if="scope.row.KeyType == 'enc'">加密</span>
                        <span v-if="scope.row.KeyType == 'sig'">签名</span>
                    </template>
                </el-table-column>
                <el-table-column label="密钥长度">
                    <template slot-scope="scope">
                        <span v-if="scope.row.KeyType == 'enc'">
                            <span v-if="scope.row.UIKeyLen_enc==null|| scope.row.UIKeyLen_enc =='0'">-</span>
                            <span v-else>{{scope.row.UIKeyLen_enc}}</span>
                        </span>
                        <span v-if="scope.row.KeyType == 'sig'">
                            <span v-if="scope.row.UIKeyLen_sig == null|| scope.row.UIKeyLen_sig =='0'">-</span>
                ...
                <el-table-column label="操作">
                    <template slot-scope="scope">
                        <div v-if="scope.row.KeyType == 'enc'">
                            <el-button @click.native.stop="addSymmetricKey(1,scope.row)" v-if="scope.row.UIKeyLen_enc ==null || scope.row.UIKeyLen_enc =='0'" type="text" size="mini" class="el-icon-plus" style="color: #409EFF">生成</el-button>
                            ...
                            <el-button @click.native.stop="xiangqing(scope.row,3)" :disabled="scope.row.UIKeyLen_enc ==null|| scope.row.UIKeyLen_enc =='0'" type="text" size="mini" class="el-icon-edit" style="color: #67C23A">修改访问码</el-button>
                            <el-button @click.native.stop="jumpToUser(scope.row,'enc')" :disabled="scope.row.UIKeyLen_enc ==null|| scope.row.UIKeyLen_enc =='0'" type="text" size="mini" class="el-icon-key" style="color: #409EFF">用户密钥</el-button>
                            <el-button :disabled="scope.row.UIKeyLen_enc ==null|| scope.row.UIKeyLen_enc =='0'" @click.native.stop="deleteKey(scope.row)" type="text" size="mini" class="el-icon-delete" style="color: #da0909">销毁</el-button>
                        </div>
                        <div v-if="scope.row.KeyType == 'sig'">
                            <el-button @click.native.stop="addSymmetricKey(1,scope.row)" v-if="scope.row.UIKeyLen_sig ==null|| scope.row.UIKeyLen_sig =='0'" type="text" size="mini" class="el-icon-plus" style="color: #409EFF">生成</el-button>
                            <el-button @click.native.stop="addSymmetricKey(1,scope.row)" v-else type="text" size="mini" class="el-icon-edit" style="color: #409EFF">更新</el-button>
                            <!-- <el-button @click.native.stop="imSymmetricKey(1,scope.row,'导入')" type="text" size="mini" class="el-icon-edit" style="color: #9227b5">导入</el-button> -->
                            <!-- <el-button @click.native.stop="exSymmetricKey(1,scope.row,'导出')" type="text" size="mini" class="el-icon-edit" style="color: #c51d82">导出公钥</el-button> -->
                            <el-button @click.native.stop="xiangqing(scope.row)" :disabled="scope.row.UIKeyLen_sig ==null|| scope.row.UIKeyLen_sig =='0'" type="text" size="mini" class="el-icon-edit" style="color:#67C23A">修改访问码</el-button>
                            <el-button @click.native.stop="jumpToUser(scope.row,'sig')" :disabled="scope.row.UIKeyLen_sig ==null|| scope.row.UIKeyLen_sig =='0'" type="text" size="mini" class="el-icon-key" style="color: #409EFF">用户密钥</el-button>
                            <el-button :disabled="scope.row.UIKeyLen_sig ==null|| scope.row.UIKeyLen_sig =='0'" @click.native.stop="deleteKey(scope.row)" type="text" size="mini" class="el-icon-delete" style="color: #da0909">销毁</el-button>
                ...
                <el-row :gutter="20" style="margin-top:10px" v-if="Isbatch!=1">
                    <el-col :offset="2">
                        <el-form-item label="密钥用途:" label-width="17%" prop="type">
                            <el-radio style="margin-left:10px" v-model="form.type" label="sig_sm9_master">签名</el-radio>
                            <el-radio v-model="form.type" label="enc_sm9_master">加密</el-radio>
                            <el-radio v-model="form.type" label="sm9_master" v-if="isImport !='1' && isImport !='4'">签名和加密</el-radio>
                        </el-form-item>
                    </el-col>
                </el-row>
...
export default {
    ...
    created() {
        ...
        this.$commonJs.deviceConfGet().then((res) => {
            this.total = res.data.data.sm9_Master_num
        })
    ...
    methods: {
        ...
        getKeyState(type) {
            ...
            this.SM9MasterkeyList = []
            this.$commonJs
                .getMethodData(this.$url.GetSM9MasterKeyState, "POST", {
                    // keynum *密钥个数
                ...
                .then((res) => {
                    if (res.data.code == 100000) {
                        ...
                        // this.SM9MasterkeyList = res.data.data;
                        dataList.forEach((element) => {
                            this.SM9MasterkeyList.push(
                                {
        ...
        addSymmetricKey(type, row) {
            ...
            if (type == 1) {
                ...
                if (this.rowList.KeyType == "sig") {
                    // 签名
                    this.form.type = "sig_sm9_master"
                } else {
                    // 加密
                    this.form.type = "enc_sm9_master"
                }
            }
        },
        ...
        deleteKey(row) {
            this.$confirm("是否永久销毁SM9主密钥?", "提示", {
                ...
                .then((res) => {
                    ...
                    this.$commonJs
                        .getMethodData(this.$url.DelMasterSM9Key, "POST", {
                            //密钥用途：1签名/2加密
                            uiSignFlag: dataVal,
                            // uiKeyIndex - 密钥内部索引号
                            masterIndex: row.KeyIndex
                        })
                        .then((res) => {
                            if (res.data.code == 100000) {
                                console.log(res, "res")
        ...
        xiangqing(row) {
            ...
            if (row.KeyType == "sig") {
                // 签名
                this.modifyForm.type = "sig_sm9_master"
            } else {
                this.modifyForm.type = "enc_sm9_master"
            }
