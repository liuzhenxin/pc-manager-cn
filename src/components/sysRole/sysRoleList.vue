            ...
            'font-size': '14px',
                ...
                <el-table-column label="系统类型" prop="system_type">
                    <template slot-scope="scope">
                        ...
                        <span v-else-if="scope.row.system_type==40">KMS</span>
                        <span v-else>一体机</span>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        getRolesList() {
            ...
            // let data = form;
            this.$commonJs
                .getMethodData(this.$url.GetRolesPresetList, "POST", this.form)
                .then((res) => {
        ...
        getTreeByRoleId(row) {
            if (row.menu_ids != null) {
                let treeData = row.menu_ids.split(",")
                this.treeCheckedKeys(this.$refs.tree, this.menuList, treeData)
                // this.$refs.tree.setCheckedKeys(treeData)

                this.roleMenu = treeData
            } else {
                this.$refs.tree.setCheckedKeys([])
                this.roleMenu = []
            }
        },
        treeCheckedKeys(tree, menuList, checkedIds) {
            let treeMap = new Map()
            let checkedData = []
            for (let i = 0; i < menuList.length; i++) {
                treeMap.set(menuList[i].id, 1)
                if (menuList[i].children.length != 0) {
                    menuList[i].children.forEach((item) => {
                        treeMap.set(item.id, 2)
                    })
                }
            }
            for (let i = 0; i < checkedIds.length; i++) {
                if (
                    treeMap.get(parseInt(checkedIds[i])) == 2 ||
                    checkedIds[i] == "1"
                ) {
                    checkedData.push(checkedIds[i])
                }
            }
            tree.setCheckedKeys(checkedData)
        },
        addMenu() {
            if (this.roleMenu.length != 0 && this.ischeckList.length < 1) {
                this.$message.warning("权限已存在,无须重复分配!")
                this.dialogVisible = false
                return false
            } else if (
                this.roleMenu.length == 0 &&
                this.ischeckList.length < 1
            ) {
        ...
        handleDelete(row) {
            this.$confirm("是否永久删除此角色?", "提示", {
                ...
                .then(() => {
                    this.$commonJs
                        .getMethodData(this.$url.DelRolesPreset, "POST", {
                            id: row.id
                        })
                        .then((res) => {
                            if (res.data.code == 100000) {
