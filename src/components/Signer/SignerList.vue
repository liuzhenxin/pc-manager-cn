        ...
        <el-card class="search">
            <el-row style="margin-left:-5px">
                ...
                <el-col :span="18">
                    ...
                    <el-button type="primary" size="mini" style="margin-left:15px;float:right;" @click="getSignerList">查询</el-button>
                </el-col>
            </el-row>
            ...
            <el-row style="margin-top:15px;">
                <el-col>
                    <el-button size="mini" @click="jumpToInsert()" type="primary" icon="el-icon-plus">添加签名者</el-button>
                </el-col>
            </el-row>
        </el-card>
        <!-- 列表 -->
        <el-card>
            <el-table ref="multipleTable" v-loading="loading" :data="SignerData" tooltip-effect="dark" :header-cell-style="{
            'text-align': 'center',
            'color': '#333',
            'font-size': '14px',
                ...
                <el-table-column label="类型">
                    <template slot-scope="scope">
                        <span style="" v-if="scope.row.type=='soft'">软件</span>
                        <span style="" v-else-if="scope.row.type=='cryptocard'">密码卡</span>
                    </template>
                </el-table-column>
                <el-table-column label="用途">
                    <template slot-scope="scope">
                        <span style="" v-if="scope.row.purpose=='cert'">证书</span>
                        <span style="" v-else-if="scope.row.purpose=='crl'">吊销</span>
                    </template>
                </el-table-column>
                <el-table-column label="签名算法名称" prop="sig_alg_name"></el-table-column>
                <el-table-column label="厂家名称" prop="factory_name"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        getSignerList() {
            ...
            this.loading = true;
            this.$commonJs
                .getMethodData(this.$url.SignerList, "POST", this.form)
                .then((res) => {
                    if (res.data.code == 100000) {
                        this.SignerData = res.data.page.receList;
        ...
        handleDelete(row, type) {
            ...
            if (type == "删除") {
                this.$confirm("是否永久删除此条签名者?", "提示", {
                    ...
                    .then(() => {
                        this.$commonJs
                            .getMethodData(this.$url.SignerDel, "POST", {
                                id: row.id
                            })
                            .then((res) => {
                                if (res.data.code == 100000) {
